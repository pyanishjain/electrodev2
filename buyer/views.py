...
def proddetails(request,id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return HttpResponse("item alreay in cart")
        return redirect('buyer:cartitems')
    # return render(request,'viewproduct.html',{'pro':p,'cat':c})
    return render(request,'sinrato_viewproduct.html',{'pro':p,'cat':c})


@login_required
def cartitems(request):
    buyerobj = UserProfile.objects.get(user__id = request.user.id)
    order=buyerorder.objects.filter(buyerid=buyerobj)
    ...
    return render(request,'sinrato_showcart.html',{'cart':car,'total':total,'count':count,'cat':c})
...
def addDeliveryDetails(request,id):
    ...
    # return render(request,'sinrato_prodDeliveryDetails.html')
...
def checkout(request,id):
    if request.method == 'POST':
        ...
        ob = buyerorder(userid = buyerobj,buyerid = request.user,prodcartid=prodobj.id,productname=prodobj.product_name,productprice=prodobj.product_price,productqty=qty[0],buyermobile=buyerobj.mobile,buyeraddress=add[0],productimage=prodobj.product_img,status="ordered",tim=now)
        ob.save()

        shoppcart.objects.filter(buyerid=request.user).filter(prodcartid=prodobj.id).delete()
        
        # return HttpResponse(remove_cart_item)
    ...
    return HttpResponse("Why you get this page chal bhag")
...
@login_required
def order(request):
    uobj= UserProfile.objects.get(user__username = request.user.username)
    obj = buyerorder.objects.filter(buyerid = uobj).order_by('-tim')
...
def buyallproduct(request):
    ...
    if request.method == 'POST':
        ...
        re=str(address)+str(quantity)+str(ids)  
    ...
    # return render(request,'sinrato_buyallproduct.html',{'obj':obj,'address':aobj})
